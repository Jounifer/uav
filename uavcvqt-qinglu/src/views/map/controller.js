...
export default class Controller {
  ...
  initRegionLayer() {
    ...
    this.layer.region = new Layer({
      ...
      style(feature) {
        ...
        return new Style({ image: new Icon({ src: feature.get('icon') }) })
  ...
  initSectorLayer() {
    ...
    this.layer.sector = new Layer({
      ...
      style() {
        ...
        return new Style({ fill: new Fill({ color: 'rgba(233, 57, 85, .55)' }) })
  ...
  addOrSetRelay(node) {
    ...
    const {
      height, id, longitude, latitude, ...options
    } = node
  ...
  addOrSetSector(node) {
    ...
    const {
      id, longitude, latitude, angle,
    } = node
    const feature = this.vector.sector.getFeatureById(id)
    if (feature) {

      feature.setProperties({
        geometry: new Polygon(createSectorPoint([
          longitude,
          latitude,
        ], 120, angle, 30)),
      })
      ...
      this.vector.sector.addFeature(createSector([
        longitude,
        latitude,
      ], 120, angle, 30, {
        id,
        type: Controller.Type.Sector,
      }))
